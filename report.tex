...
decodes the payload. Everything is written from scratch; no QR-specific detection/decoding library
...
All binarizations in Phases 1--5b use the scaled working image. Phase~1 always runs; later
...
All per-strategy pools are concatenated. In \texttt{\_merge\_candidates},
the parameter \texttt{merge\_factor} is set to $0.6$. The merge radius is
\[
r_{ij} = 0.6 \cdot \max\!\left(\sqrt{\mathrm{area}_i}, \sqrt{\mathrm{area}_j}\right).
\]
If Euclidean distance between centres is below this threshold, the two
candidates are assigned to the same cluster. Greedy processing in
descending \texttt{size} keeps one representative per cluster. The merged
list is capped at $300$ entries before triplet search.
...
+ 60\,\sigma_{\text{size}} .
\]
\textit{Note:} Since $r_{\text{side}}=\max(d_1,d_2)/\min(d_1,d_2)\ge1$ in this code path, $(r_{\text{side}}-1)$ is non-negative (equivalent to $|r_{\text{side}}-1|$), identical to \texttt{main.py}.
...
contains more than three finder centres in total are always rejected. Finders
...
Reed--Solomon arithmetic is performed in $GF(2^8)$ using the QR-standard primitive polynomial $x^8 + x^4 + x^3 + x^2 + 1$ (hex \texttt{0x11D}). The implementation comprises:
...
skipped rather than implemented, which is sufficient for the vast
...
Detection runtime in the measured public runs is 14 ms/image (train) and 20 ms/image (valid) without decoding.
...
Decoding can be disabled with \texttt{---decode=no}.
...
Re-running the evaluation on the train or valid split gives
identical \texttt{output.csv} files (no randomness, no
